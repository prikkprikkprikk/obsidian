---
Kunde:
  - "[[Databaser/Kundar/CorePublish\\|CorePublish]]"
Prosjektstatus: På vent
Prosjekttype: CoreTrek
✔️ Oppgåver:
  - "[[Hente liste med kolonner]]"
---
#### Oppgåver

|![](https://www.notion.so/icons/font_gray.svg)Name|![](https://www.notion.so/icons/clock_gray.svg)Date Created|![](https://www.notion.so/icons/calendar_gray.svg)Deadline|![](https://www.notion.so/icons/arrow-circle-down_gray.svg)Hast|![](https://www.notion.so/icons/arrow-northeast_gray.svg)Hovudoppgåve|![](https://www.notion.so/icons/checkmark-square_gray.svg)Innboks|![](https://www.notion.so/icons/arrow-northeast_gray.svg)Områder|![](https://www.notion.so/icons/arrow-northeast_gray.svg)Prosjekt|![](https://www.notion.so/icons/arrow-circle-down_gray.svg)Status|![](https://www.notion.so/icons/arrow-circle-down_gray.svg)Type|![](https://www.notion.so/icons/arrow-northeast_gray.svg)Underoppgåver|
|---|---|---|---|---|---|---|---|---|---|---|
|[[Hente liste med kolonner]]|11. september 2024 11:10||Ein vakker dag||[ ]|[[Jobb]]|[[Multiselect]]|På vent|CoreTrek||

  
  

  

  

Har lagt inn jQuery-biblioteket **Multiselect**:

  

> [!info] jQuery multiselect plugin with two sides  
> jQuery multiselect plugin with two sides.  
> [https://crlcu.github.io/multiselect/#documentation](https://crlcu.github.io/multiselect/#documentation)  

  

## CtArticleElementSelectValuesProvider

Interface: cplib/article/element/CtArticleElementSelectValuesProvider.php

  

## Original case

Banksikringsfondet trengde brukarliste med andre kolonner enn det som er standard i «Ansattliste».

Vi enda med å hardkode kolonnene i det tilfellet, men det kunne kanskje vere verdt å undersøke om det går an å lage noko slikt til bruk i andre prosjekt.

![[📎 Filer/image 5.png|image 5.png]]